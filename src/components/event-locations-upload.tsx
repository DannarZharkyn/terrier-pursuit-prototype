...
import {
  parseEventLocationImportFile,
  type EventLocationImportResult,
} from "@/lib/imports/event-locations";

export function EventLocationsUpload({
  onResultChange,
}: {
  onResultChange?: (result: EventLocationImportResult | undefined) => void;
}) {
  const inputRef = useRef<HTMLInputElement>(null);
  const [fileName, setFileName] = useState<string>();
  const [result, setResult] = useState<EventLocationImportResult>();
  ...
  async function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const nextResult = parseEventLocationImportFile(fileData);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold text-gray-900">
              Upload Locations & Clues Spreadsheet
            </p>
            <p className="text-sm text-gray-500">
              {fileName
                ? fileName
                : "Click to browse for Landmark, Location, Clue, Campus Population"}
            </p>
          </div>
        </div>
      </button>
      ...
      {result?.ok ? (
        <div className="mt-3 rounded-lg border border-green-200 bg-green-50 p-4">
          <div className="flex items-center gap-2 text-sm font-bold text-green-800">
            ...
            Locations and clues look good.
          </div>
          <dl className="mt-3 grid gap-2 text-sm text-green-900 sm:grid-cols-3">
            <div>
              <dt className="font-semibold">Locations found</dt>
